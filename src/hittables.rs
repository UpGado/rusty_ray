...
pub enum HitResult {
    Scatter(Vec3, f64, Ray) // color, tranmission, new_ray
}
...
impl Hittable for Hittables {
    fn hits(&self, ray: Ray, t_min: f64, t_max: f64) -> Option<Hit> {
        self.iter().map(|h| h.hits(ray, t_min, t_max))
            .filter_map(|p| p).min_by(|x, y| x.time.partial_cmp(&y.time)
...
impl Hittable for Sphere {
    fn hits(&self, ray: Ray, t_min: f64, t_max: f64) -> Option<Hit> {
        ...
        let c = (shifted_ray_origin).dot(&shifted_ray_origin)
                - self.radius.powi(2);
        ...
        if term_under_sqrt > 0.0 {
            ...
            for &t in ts.iter() {
                if (t < t_min) || (t > t_max) {
                    ...
                    return Some(Hit{
                        result:self.material.get_hit_result(point, ray.direction, outward_normal)
                        , time: t});
                }
            };
        }
